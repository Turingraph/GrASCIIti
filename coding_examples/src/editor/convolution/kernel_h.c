#include"convolution.h"

int	main(int len, char **str)
{
	t_table_fdf	table_a;
	t_table_fdf	table_b;
	double		**kernel;
	size_t		half_dim;
	int			output;

	if (len < 3)
		return (0);
	output = open_dir_file(str[2], NULL, APPEND);
	half_dim = 3;
	table_a = open_table_fdf_file(str[1], NULL, cheche01_ascii_line, FALSE);
	table_b = scale_dimension_fdf(table_a, 3, 3);
	free_table_fdf(&table_a);
	kernel = blur_kernel_2d(2 * half_dim + 1, 1.0 / f_pow(2.0 * (double)half_dim + 1.0, 2));
	table_a = convolution_hight(&table_b, kernel, half_dim);
	write_table_ascii_cheche01(output, &table_a, D5_HEIGHT);
	free_table_fdf(&table_a);
	free_table_fdf(&table_b);
	free_2d_arr((void **)kernel, 2 * half_dim + 1);
	return (0);
}

