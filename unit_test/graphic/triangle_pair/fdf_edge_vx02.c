...
int	main(void)
{
	...
	t_triangle_tester	testers[] = {
		...
		{
			// ***
			.row = 6, .col = 8, .length = 2, .file_name = "basictest.fdf", .points = (t_3d_point []){
		...
		{
			// ***
			.row = 1, .col = 1, .length = 2, .file_name = "elem.fdf", .points = (t_3d_point []){
				{.x = 1.0, .y = 1.0, .z = 10.0}, {.x = 2.0, .y = 1.0, .z = 10.0}
			}
		},
		{
			// ***
			.row = 9, .col = 4, .length = 2, .file_name = "elem2.fdf", .points = (t_3d_point []){
	...
	while (i < max_score)
	{
		table = open_table_fdf_file(testers[i].file_name, "input_examples/fdf_tab/", parse_fdf_line_rgba, false);
...
/*
valgrind --leak-check=full --show-leak-kinds=all ./unit_test/out/graphic/triangle_pair/fdf_edge_vx02.out
*/