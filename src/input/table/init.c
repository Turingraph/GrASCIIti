...
// time : O(n)
// space: O(1)
void	load_and_save_rgb(const t_load_fdf_arr *src, t_table_fdf *dst, size_t row, size_t col)
{
	if (row < dst->row && col < dst->col && row < src->length
		&& src->arr != NULL && col < src->arr[row].length)
	{
		if (choose_rgb_channel(dst, RED, row) != NULL && src->arr[row].r != NULL)
			dst->r[row][col] = src->arr[row].r[col];
		if (choose_rgb_channel(dst, GREEN, row) != NULL && src->arr[row].g != NULL)
			dst->g[row][col] = src->arr[row].g[col];
		if (choose_rgb_channel(dst, BLUE, row) != NULL && src->arr[row].b != NULL)
			dst->b[row][col] = src->arr[row].b[col];
		if (choose_rgb_channel(dst, ALPHA, row) != NULL && src->arr[row].a != NULL)
...
t_table_fdf	load_table_fdf(const t_load_fdf_arr *src, e_bool is_rgb)
{
	...
	if (src == NULL || src->length == 0 || src->capacity == 0)
