	...
	t_2d_int (*projection)(float x, float y, float z))
{
	...
	point = projection(
			...
			get_fdf_point(fdf, ixiy, 3, 0));
	line.p1 = world_3d_to_screen_2d(camera, point.x, point.y);
	point = projection(
			get_fdf_point(fdf, ixiy, 1, 2),
			get_fdf_point(fdf, ixiy, 2, 2),
			get_fdf_point(fdf, ixiy, 3, 2));
	line.p2 = world_3d_to_screen_2d(camera, point.x, point.y);
	return (line);
}
