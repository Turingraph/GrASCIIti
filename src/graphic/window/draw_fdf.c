	...
	bool is_draw, t_2d_int ixiy)
{
	...
	line.p2 = world_to_screen_3d(*hook->camera,
		hook->master_piece.still_life->pos_x[(ixiy.y + 1) * hook->master_piece.still_life->src->col + ixiy.x],
		hook->master_piece.still_life->pos_y[(ixiy.y + 1) * hook->master_piece.still_life->src->col + ixiy.x]);
	...
	bool is_draw, t_2d_int ixiy)
{
	...
	line.p1 = world_to_screen_3d(*hook->camera,
		...
		hook->master_piece.still_life->pos_y[ixiy.y * hook->master_piece.still_life->src->col + ixiy.x]);
	line.p2 = world_to_screen_3d(*hook->camera,
		hook->master_piece.still_life->pos_x[ixiy.y * hook->master_piece.still_life->src->col + ixiy.x + 1],
		hook->master_piece.still_life->pos_y[ixiy.y * hook->master_piece.still_life->src->col + ixiy.x + 1]);
	draw_fdf_mlx_unit(line, ink, *(hook->camera), hook->img);
	// write_line(line);
	// write(1, "\t", 1);
}
