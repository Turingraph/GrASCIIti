...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/08/24 22:43:11 by phsottat          #+#    #+#             */
/*   Updated: 2026/08/24 22:53:35 by phsottat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct t_fline
{
	t_complex	p1;
	t_complex	p2;
}	t_fline;

typedef struct t_tile_format
{
	float		tile_size;
...
typedef struct t_motif
{
	t_2d_polygon	polygon;
	t_ink32			ink;
}	t_motif;

typedef struct t_motif_arr
{
	t_motif	*arr;
	size_t	length;
}	t_motif_arr;
...
typedef enum t_edrawing_target
{
	E_MINIMALISM,
	E_MOTIF,
	E_STILL_LIFE
...
// init.c
float			init_fdf_position_unit(const t_table_fdf *src,
					char axis, size_t i);
...
// linalg.c
float			linear_map_3d(const t_fdf *src,
					t_matrix matrix, size_t i, char axis);
...
bool			is_2dhook_valid(const t_2d_hook *src,
					t_edrawing_target drawing_target);
...
t_2d_int		world_to_screen_3d(t_2d_camera camera,
					float x, float y);
bool			is_line_in_screen(t_2d_camera camera, t_2d_int src);
